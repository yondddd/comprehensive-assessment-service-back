package com.lm.assessment.common.utils.json.json.manager;
...
public final class JsonTypeConvertManager {
    ...
    private static TypeFactory FACTORY = JsonUtils.getDefaultMapper().getMapper().getTypeFactory();
    ...
    private static Map<Field, Type> cache = new HashMap<>();

    public static Type getFieldGenericType(@NotNull Field field) {
    ...
    private synchronized static Type getFieldGenericType0(@NotNull Field field) {
        Type javaType = cache.get(field);
        if (null != javaType) {
            return javaType == NULL ? null : javaType;
        }
    ...
    private static Optional<Type> getFieldGenericTypeByGson(@NotNull Field field) {
